...
from .models import Post, Category, MPost, MCategory
from .forms import CommentForm, PostForm, CategoryForm, MCommentForm, MPostForm
from settings.models import Setting, MSetting
...
def get_mcategory_count():
    category_query = MPost.objects.values('categories__slug', 'categories__title').annotate(Count('categories__title'))
    return category_query
...
class MListView(ListView):
    model = MPost
    template_name = 'blog/market.html'
    context_object_name = 'mposts'
    ordering = ['-timestamp']
    paginate_by = 8

    def get_context_data(self, *args, **kwargs):
        category_count = get_mcategory_count()
        most_recent = MPost.objects.order_by('-timestamp')[:3]
        settings = MSetting.objects.last()
        context = super().get_context_data(**kwargs)
        context['most_recent'] = most_recent
        context['settings'] = settings
        context['category_count'] = category_count
...
class MDetailView(DetailView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            form.instance.post = post
            form.save()
            return redirect(reverse('mpost_detail', kwargs={'slug': post.slug}))

class MCreateView(LoginRequiredMixin, CreateView): #edit
    model = MPost
    template_name = 'blog/market_form.html'
    form_class = MPostForm
    permission_required = 'blog.fields'
...
class MPostDeleteView(LoginRequiredMixin, UserPassesTestMixin, DeleteView):
    model = MPost
...
class MUpdateView(LoginRequiredMixin, UserPassesTestMixin, UpdateView):
    model = MPost
    form_class = MPostForm
